 ...
 *
 * Reads markdown files from two source folders and generates TypeScript
 * data files for the Nebula sections.
 *
 * Sources:
 ...
 *
 * OUTPUT FORMATS:
 * - Scratch Pad: Single TypeScript file with full content (small dataset)
 * - Knowledge Base: JSON files in /public/kb/ + slim TypeScript index (performance optimized)
 */
...
const SOURCES = {
  scratchPad: {
    ...
    outputFile: path.join(__dirname, '..', 'src', 'data', 'scratch-pad.ts'),
    ...
    outputMode: 'full', // Include full content in TypeScript
  },
  knowledgeBase: {
    name: 'Knowledge Base',
    sourceDir: path.join(__dirname, '..', 'gemini-responses'),
    outputFile: path.join(__dirname, '..', 'src', 'data', 'knowledge-base-index.ts'),
    jsonOutputDir: path.join(__dirname, '..', 'public', 'kb'),
    interfaceName: 'KnowledgeBaseDocMeta',
    exportName: 'knowledgeBaseIndex',
    getterName: 'getDocMeta',
    slugsGetterName: 'getAllKnowledgeBaseSlugs',
    titleExtractor: extractTitleFromFrontmatter, // Simple frontmatter extraction
    outputMode: 'split', // JSON files + slim TypeScript index
  },
};
...
/**
 * Generate TypeScript file with FULL content (for Scratch Pad)
 */
function generateFullTypeScriptFile(documents, config) {
  return `/**
...
/**
 * Generate slim TypeScript INDEX file (metadata only, no content)
 * Content is loaded on-demand from /public/kb/{slug}.json
 */
function generateMetadataIndexFile(documents, config) {
  // Create metadata-only array (exclude content field)
  const metadataOnly = documents.map(({ slug, title, date, sourceFile }) => ({
    slug,
    title,
    date,
    sourceFile,
  ...
  return `/**
 * ${config.name} Index (Metadata Only)
 *
 * Auto-generated by scripts/sync-nebula-docs.js
 * Source: ${config.sourceDir}
 * Generated: ${new Date().toISOString()}
...
export interface KnowledgeBaseDocFull extends ${config.interfaceName} {
  content: string;
}

export const ${config.exportName}: ${config.interfaceName}[] = ${JSON.stringify(metadataOnly, null, 2)};
...
/**
 * Write individual JSON files to /public/kb/
 */
function writeKnowledgeBaseJsonFiles(documents, jsonDir) {
  // Ensure directory exists
  if (!fs.existsSync(jsonDir)) {
    fs.mkdirSync(jsonDir, { recursive: true });
  }

  // Clear existing JSON files (to remove stale documents)
  const existingFiles = fs.readdirSync(jsonDir).filter(f => f.endsWith('.json'));
  for (const file of existingFiles) {
    fs.unlinkSync(path.join(jsonDir, file));
  }

  // Write each document as individual JSON file
  for (const doc of documents) {
    const jsonPath = path.join(jsonDir, `${doc.slug}.json`);
    fs.writeFileSync(jsonPath, JSON.stringify(doc, null, 2), 'utf-8');
  }

  console.log(`  Written ${documents.length} JSON files to ${jsonDir}`);
}
...
function syncSource(config) {
  ...
  if (config.jsonOutputDir) {
    console.log(`JSON Output: ${config.jsonOutputDir}`);
  }
  console.log('');
  ...
  let output;
  if (config.outputMode === 'split') {
    // Knowledge Base: JSON files + slim TypeScript index
    writeKnowledgeBaseJsonFiles(documents, config.jsonOutputDir);
    output = generateMetadataIndexFile(documents, config);
  } else {
    // Scratch Pad: Full TypeScript file with content
    output = generateFullTypeScriptFile(documents, config);
  }
